...
class TransformerBlock(nn.Module):
    def __init__(self, embedims, num_heads, ff_dim, dropout_rate=0.1):
        super().__init__()
        self.att = nn.MultiheadAttention(
            embedims=embedims, num_heads=num_heads, dropout=0.0, batch_first=True
        )
        self.ffn = nn.Sequential(
            nn.Linear(embedims, ff_dim),
            nn.GELU(),
            nn.Linear(ff_dim, embedims),
        )
        self.norm1 = nn.LayerNorm(embedims, eps=1e-6)
        self.norm2 = nn.LayerNorm(embedims, eps=1e-6)
...
class FTTransformer(nn.Module):
    ...
    def __init__(self, num_features, embedims=64, num_heads=8, ff_dim=256,
                 num_blocks=3, dropout_rate=0.2):
        super().__init__()
        self.num_features = num_features
        self.embedims = embedims

        # One projection per feature (feature tokenizer), not a single
        # shared Linear applied to all features identically.
        self.feature_projections = nn.ModuleList(
            [nn.Linear(1, embedims) for _ in range(num_features)]
        )
        self.cls_token = nn.Parameter(torch.randn(1, 1, embedims) * 0.02)

        self.transformer_blocks = nn.ModuleList(
            [TransformerBlock(embedims, num_heads, ff_dim, dropout_rate)
             for _ in range(num_blocks)]
        )
        self.norm = nn.LayerNorm(embedims, eps=1e-6)
    ...
    def forward(self, inputs):
        ...
        for i in range(self.num_features):
            feat = inputs[:, i:i + 1] 
            tok = self.feature_projections[i](feat) 
            tokens.append(tok.unsqueeze(1))
        x = torch.cat(tokens, dim=1)

        cls = self.cls_token.expand(batch_size, 1, self.embedims)
...
def tune_transformer(x_train, y_train, x_val, y_val, n_trials=25):
    def objective(trial):
        params = {
            "embedims": trial.suggest_categorical("embedims", [32, 64, 96, 128]),
        ...
        if params["embedims"] % params["num_heads"] != 0:
