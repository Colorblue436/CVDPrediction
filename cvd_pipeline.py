...
Requires: pip install xgboost lightgbm optuna tensorflow scikit-learn pandas numpy matplotlib
"""
...
class FTTransformer(models.Model):
    """
    ...
    def __init__(self, num_features, embed_dim=64, num_heads=8, ff_dim=256,
        ...
        # shared Dense applied to all features identically.
        self.feature_projections = [
            layers.Dense(embed_dim) for _ in range(num_features)
        ]
    ...
    def call(self, inputs, training=False):
        ...
        for i in range(self.num_features):
            ...
            tokens.append(tf.expand_dims(tok, axis=1))  # (batch, 1, embed_dim)
        x = tf.concat(tokens, axis=1)  # (batch, num_features, embed_dim)
...
if __name__ == "__main__":
    ...
    transformer_probs = final_transformer.predict(x_test, verbose=0).ravel()
    ...
    oof_transformer = final_transformer.predict(x_train, verbose=0).ravel()
